...
public class DataMethods {
	...
	public static double getNeighbourCellValue(LiveCell liveCell, Directions direction) {
		...
		case UP -> {
			if (liveCell.getY()>1) {
				...
				return getNextCellValue(nextCell,liveCell);
			} else {
				return WALL_VALUE;
			}
		}
	...
	public static double getNextCellValue(Cell nextCell, LiveCell liveCell) {
		LiveCell nextLiveCell=nextCell.liveCell;
		if (nextLiveCell!=null) {
			boolean isRelative=nextLiveCell instanceof NormCell
					? isRelatives(liveCell.getHead(),(NormCell) nextLiveCell)
					: false;
	...
	private static byte getChromosomeNum(byte[] curCellChromosome, Directions direction) {
		...
		case DOWN -> {
			return curCellChromosome[1];
		}

		case LEFT -> {
			return curCellChromosome[2];
		}

		case RIGHT -> {
			return curCellChromosome[3];
		}
		default -> {
			try {
				throw new Exception("Wrong direction or null");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
